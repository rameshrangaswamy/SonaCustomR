...
@Rule(key = "AvoidTooManyParametersForMethod", description = "Method should not have more than 5 parameters", priority = Priority.MAJOR,
tags = {"bug"} )
public class AvoidTooManyParametersForMethodRule extends IssuableSubscriptionVisitor{
	...
	public void visitNode(Tree tree) {
		...
		if (methodSymbol.parameterTypes().size() > 5) {
			reportIssue(tree,"Too Many Paramaeters");
		}
